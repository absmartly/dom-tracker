...
function makeConfig(overrides: Partial<DOMTrackerConfig> = {}): DOMTrackerConfig {
  return {
    onEvent: jest.fn(),
    ...overrides,
  };
...
describe('DOMTracker', () => {
  ...
  describe('emit', () => {
    it('calls onEvent handler when emit is invoked', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ onEvent, trackers: [tracker] }));
    ...
    it('calls multiple onEvent handlers', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ onEvent: [handler1, handler2], trackers: [tracker] }));
    ...
    it('continues calling remaining handlers if one throws', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ onEvent: [badHandler, goodHandler], trackers: [tracker] }));
  ...
  describe('setAttributes', () => {
    it('calls onAttribute handler when setAttributes is invoked', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ onAttribute, trackers: [tracker] }));
    ...
    it('calls multiple onAttribute handlers', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ onAttribute: [handler1, handler2], trackers: [tracker] }));
    ...
    it('continues calling remaining attribute handlers if one throws', () => {
      const badHandler = jest.fn(() => { throw new Error('attr fail'); });
      const goodHandler = jest.fn();
      let capturedContext: TrackerContext;
      const tracker = makeTracker({
        init(ctx) {
      ...
      const domTracker = new DOMTracker(makeConfig({ onAttribute: [badHandler, goodHandler], trackers: [tracker] }));
  ...
  describe('tracker registration', () => {
    ...
    it('catches tracker init errors and continues with other trackers', () => {
      const badTracker = makeTracker({
        name: 'bad',
        init: jest.fn(() => { throw new Error('init fail'); }),
      });
      const goodTracker = makeTracker({ name: 'good' });

      expect(() => new DOMTracker(makeConfig({ trackers: [badTracker, goodTracker] }))).not.toThrow();
    ...
    it('catches tracker destroy errors during removeTracker', () => {
      const tracker1 = makeTracker({
        name: 'tracker1',
        destroy: jest.fn(() => { throw new Error('destroy fail'); }),
      });
  ...
  describe('destroy', () => {
    it('calls destroy on all registered trackers', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ trackers: [tracker1, tracker2] }));
      domTracker.destroy();
  ...
  describe('TrackerContext', () => {
    it('getConfig returns the config', () => {
      ...
      const tracker = makeTracker({ init(ctx) { capturedCtx = ctx; } });
    ...
    it('uses custom pageName function when provided', () => {
      ...
      let capturedCtx: TrackerContext;
      const tracker = makeTracker({ init(ctx) { capturedCtx = ctx; } });
      const domTracker = new DOMTracker(makeConfig({ pageName, trackers: [tracker] }));
  ...
  describe('SPA mode', () => {
    it('creates SPAObserver when spa: true and calls onRouteChange on trackers', () => {
      ...
      const domTracker = new DOMTracker(makeConfig({ spa: true, trackers: [tracker] }));
