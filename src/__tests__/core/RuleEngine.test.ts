...
describe("RuleEngine", () => {
  ...
  it("binds click events for rules and emits on click", () => {
    ...
    expect(emit).toHaveBeenCalledWith("cta_clicked", {
      page_name: "test-page",
    });
  });
  ...
  it("merges rule props into emitted payload", () => {
    ...
    engine.addRule({
      selector: ".cta",
      event: "cta_clicked",
      props: { campaign: "summer", position: 1 },
    });
    engine.bind();
  ...
  it("uses custom event type via rule.on", () => {
    ...
    expect(emit).toHaveBeenCalledWith("search_focused", {
      page_name: "test-page",
    });
  });
  ...
  it("addRule can be called after bind and then rebind picks it up", () => {
    document.body.innerHTML =
      '<button class="btn1">A</button><button class="btn2">B</button>';
    ...
    expect(emit).toHaveBeenCalledWith("btn1_clicked", {
      page_name: "test-page",
    });
    expect(emit).toHaveBeenCalledWith("btn2_clicked", {
      page_name: "test-page",
    });
  });
  ...
  it("skips elements with data-abs-track attribute", () => {
    document.body.innerHTML =
      '<button class="cta" data-abs-track="already_tracked">Buy</button>';
    engine.addRule({ selector: ".cta", event: "cta_clicked" });
    engine.bind();
  ...
  it("handles multiple matching elements", () => {
    document.body.innerHTML =
      '<button class="btn">A</button><button class="btn">B</button>';
  ...
  it("tracks dynamically added elements via delegation", () => {
    ...
    expect(emit).toHaveBeenCalledWith("dynamic_clicked", {
      page_name: "test-page",
    });
  });
  ...
  it("tracks clicks on child elements of matched selector", () => {
    document.body.innerHTML =
      '<a class="cta" href="/demo"><span>Get Demo</span></a>';
    engine.addRule({ selector: ".cta", event: "cta_clicked" });
    engine.bind();
  ...
  it("debounces duplicate rule events within 500ms on the same element", () => {
    jest.useFakeTimers();
    document.body.innerHTML = '<button class="cta">Buy</button>';
    engine.addRule({ selector: ".cta", event: "cta_clicked" });
    engine.bind();
